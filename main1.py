...
def gemini_json_response(prompt: str) -> dict:
    if not gemini_model:
        abort(500, description="Gemini client not initialized. Please check your API key.")
    try:
...
"""
    result = gemini_json_response(prompt)
...
def health_check():
    gemini_status = "connected" if gemini_model else "not connected"
    api_key_status = "present" if os.getenv("GEMINI_API_KEY") else "missing"
    return jsonify({
        "status": "healthy",
